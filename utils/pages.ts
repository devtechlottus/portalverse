...
export const getPageType = async (path: string): Promise<PageType> => {
  ...
  const isBlogEntryPage =
    path?.startsWith(`${blogEntryParentSlug}/`) &&
    normalizedPath !== normalizePath(blogEntryParentSlug);

  const levelsConfig = await getEducationalOfferingConfig();
  const programDetailParentSlugs = levelsConfig?.map((levelConfig) =>
    normalizePath(levelConfig?.slug)
  );
  const isProgramDetailPage = programDetailParentSlugs?.reduce(
    (acc, parentSlug) => {
      return (
        acc ||
        (normalizedPath?.startsWith(`${parentSlug}/`) &&
          normalizedPath !== parentSlug)
      );
    },
    false
  );
...
export const getProgramDetailPageData = async (
  path: string
): Promise<ProgramDetailPage> => {
  const pathSegments = normalizePath(path)?.split("/");
  const levelSlug = pathSegments?.slice(
    pathSegments?.length - 2,
    pathSegments?.length - 1
  )?.[0];
  const programSlug = pathSegments?.slice(
    pathSegments?.length - 1,
    pathSegments?.length
  )?.[0];
  ...
  //@ts-ignore
  const continuousEducationPrograms =
  ...
  const isStaticProgram = isContinuousEducationProgram
    ...
    : staticProgramsByLevel?.some((level) => {
        return level?.params?.programs?.some((program) => {
          return (
            level?.params?.level === levelSlug &&
            program?.params?.program === programSlug
          );
        });
      });
  ...
  if (isStaticProgram) {
    if (isContinuousEducationProgram) {
      const { sections, meta } = await getDataPageFromJSON(
        `extension-universitaria/${programSlug}.json`
      );
      ...
      const { meta, config, sections, form } = await getDataPageFromJSON(
        `/oferta-educativa/${levelSlug}/${programSlug}.json`
      );
      const bannerParche = await getDataPageFromJSON(
        "oferta-educativa/oferta-educativa.json"
      );
    ...
    const programLevel =
      programData?.attributes?.level?.data?.attributes?.title;
...
export const getJSONProgramsPaths = async () => {
  ...
  try {
    ...
    continuousEducationStaticCategories = continuousEducationStaticPageData
      ?.sections?.extension
  ...
  const hiddenContinuousEducationProgramsPaths =
    continuousEducationStaticCategories
      ?.reduce((programs, category) => {
        // merge all programs from all categories
  ...
  for (const levelParams of educationalOfferingParams) {
    const staticProgramsData = await getDataPageFromJSON(
      `/oferta-educativa/${levelParams?.params?.level}.json`
    );
    ...
    for (const program of programsParams) {
      ...
      const isProgramAvailable = !staticPrograms?.find(
        (staticProgram) => staticProgram?.route === programSlug
      )?.config?.hidden;
  ...
  continuousEducationParams?.params?.programs
    ?.filter((program) => {
      return !hiddenContinuousEducationProgramsPaths?.includes(
...
export const getProgramDetailPagesPaths = async () => {
  ...
  const paths = [
    ...strapiProgramDetailPagesPaths,
    ...staticProgramDetailPagesPaths,
  ]?.map(normalizePath);
