...
const Footer: FC<FooterPortalverseComponentData> = ({ privacyLink, certifications, logotype, social, phone, directorio, sections, onClickLogo }: FooterPortalverseComponentData) => {
  ...
  return <section className="w-full border-t-[1px] border-solid border-[#282828] mt-18 w-t:mt-12 w-p:mt-12">
    {/* Section logotype */}
              ...
              section.map((itemSection: any[], a: number) => {
                return itemSection.map((item: any, j: number) => {
                  return <Fragment key={`section-access-${j}`}>
                    {
                      !!item.link
                        ? <Link href={item.link} passHref><a target={!!item.external ? "_blank" : "_self"}><p className={cn({ "font-bold": item.principal, "font-normal": !item.principal })}>{item.label}</p></a></Link>
