...
const LinkContactTarget: FC<LinkContactTargetComponentData> = ({ type, info, classNames }: LinkContactTargetComponentData) => {
  ...
  return <a className={cn("font-Nunito font-normal text-base leading-5 text-SC/Blackandgrey/B-60", classNames)} target="_blank" rel="noreferrer" href={`${manage}${info}`}>{info}</a>
}
